...
int64_t x_tile83 = x_tile85;
int64_t y_tile84 = y_tile86;
Weights<float> input_q_18 = input.query_materialize(x81 * stride_arg80, y82 * stride_arg80, x_tile83 + x_tile83 - 1, y_tile84 + y_tile84 - 1);
Weights<float> output_tan_q = output_tan.query_materialize(x94, y97, x_tile96, y_tile95);
conv_no_fern_mkl(input_q_18, filter, bias, stride_arg80, output_conv_q);
relu_material(output_conv_q_, output_relu_q);
maxpool(output_relu_q_, maxpool_dim89, output_q);
tanh(output_q_, output_tan_q);
output_tan.insert_materialize(x94, y97, x_tile96, y_tile95, output_tan_q);
input_q_18.free_weight();
