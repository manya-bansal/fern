void my_fused_impl(const Array<float> a, Array<float> c, int64_t len48, int64_t x49, int64_t len50){
 ...
 for(int64_t x51 = c.idx;x51 < c.idx + c.size; x51+=len50){
  ...
  Array<float> c_q = c.array_query_no_materialize(x51, len50);
    ispc::addi_ispc(a_q11.data, 0.000000f, len46, out_a_q.data);
    ispc::addi_ispc(out_a_q.data, 0.000000f, len48, b_q.data);
    ispc::addi_ispc(out_a_q.data, 0.000000f, len50, c_q.data);
}
