...
public:
  convolution_mkl_test() = default;
  std::string getName() const override { return {"convolution_mkl_test"}; }

  fern::DependencySubset getDataRelationship() const override {
    fern::Variable x_tile(false, false, false);
    fern::Variable y_tile(false, false, false);
    fern::Variable stride_arg(false, false, false);
    fern::Variable x(false, false, false);
    fern::Interval x_loop(x, output["W_start"],
                          output["W_start"] + output["W_len"], x_tile);
    fern::Variable y(false, false, false);
  ...
  std::vector<fern::Argument> getArguments() override {
    return {new fern::DataStructureArg(fern::DataStructurePtr(&input)),
            fern::DataStructureArg(fern::DataStructurePtr(&filter)),
            fern::DataStructureArg(fern::DataStructurePtr(&bias)),
